...
internal static class BmsLoader
{
                    ...
                    for (var i = 0; i < objLength; i++)
                    {
                        ...
                        else
                        {
                            var noteObj = new RawNote(
                                CalculateNoteTime(tick, bpmEntries, timeSigEntries),
                                note,
                                typeCode
                            );
    ...
    private static void ProcessBossData(Bms bms)
    {
        ...
        for (var i = 0; i < bossData.Count; i++)
        {
            ...
            var bossAnimBefore = i > 0 ? bossData[i - 1] : null;
            ...
            if (bossAnimAhead != null &&
                AnimStatesLeft.TryGetValue(bossAnimAhead.noteData.boss_action, out var parsedState))
