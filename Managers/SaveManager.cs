...
public class SaveManager
{
    ...
    internal static void FixSaveFile()
    {
        ...
        if (!SaveData.UnlockedMasters.Any())
        {
            var unlockedHighest = SaveData.Highest.Where(kv =>
                    kv.Value.ContainsKey(3) && kv.Value.TryGetValue(2, out var chartSave) && chartSave.Evaluate >= 4)
                .Select(kv => kv.Key);
            var folderCharts = AlbumManager.LoadedAlbums
                .Where(kv => kv.Value.HasDifficulty(2) && kv.Value.HasDifficulty(3) && !kv.Value.IsPackaged)
                .Select(kv => kv.Key);
    ...
    private static void RestoreBackup()
    {
        ...
        {
            Logger.Fail(
                "No backups found. Please delete the CustomAlbums.json file in UserData folder to create a new save.");
            return;
        }
        ...
        Logger.Fail(
            "Could not restore save file. Please delete the CustomAlbums.json file in UserData folder to create a new save.");
    }
