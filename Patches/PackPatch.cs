...
internal class PackPatch
{
    ...
    [HarmonyPatch(typeof(LongSongNameController), nameof(LongSongNameController.Refresh), typeof(string), typeof(bool),
        typeof(float))]
    internal class RefreshPatch
    {
        private static void SetColor(string colorHex, LongSongNameController instance)
        {
            var fixedColor = ColorUtility.TryParseHtmlString(colorHex, out var color) ? color : Color.white;
        ...
        private static void Prefix(ref string text, ref Pack __state)
        {
            ...
            if (currentUid == null || !currentUid.StartsWith($"{AlbumManager.Uid}-") ||
                text != AlbumManager.GetCustomAlbumsTitle()) return;
