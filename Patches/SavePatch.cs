...
{
    internal static bool? _hqPresent;
    ...
    private static int GetDifficulty(string uid)
    {
        ...
        if (invokedHides == null || !invokedHides.ContainsKey(uid) || !hideBmsInfos.TryGetValue(uid, out var hideBms) ||
            hideBms == null) return selectedIndex;
    ...
    private static void InjectCustomData()
    {
        ...
        DataHelper.selectedMusicUidFromInfoList =
            AlbumManager.LoadedAlbums.TryGetValue(SaveData.SelectedAlbum, out var album) ? album.Uid : "0-0";
    }
    ...
    internal class LevelDesignerPatch
    {
        private static bool Prefix(MusicInfo __instance, ref string __result, int index)
        {
            ...
            {
                1 => __instance.m_MaskValue.TryGetValue("levelDesigner1", out var d1)
                    ? d1.ToString()
                    : __instance.levelDesigner1,
                2 => __instance.m_MaskValue.TryGetValue("levelDesigner2", out var d2)
                    ? d2.ToString()
                    : __instance.levelDesigner2,
                3 => __instance.m_MaskValue.TryGetValue("levelDesigner3", out var d3)
                    ? d3.ToString()
                    : __instance.levelDesigner3,
                4 => __instance.m_MaskValue.TryGetValue("levelDesigner4", out var d4)
                    ? d4.ToString()
                    : __instance.levelDesigner4,
                5 => __instance.m_MaskValue.TryGetValue("levelDesigner5", out var d5)
                    ? d5.ToString()
                    : __instance.levelDesigner5,
            ...
            if (string.IsNullOrEmpty(__result) || __result == "?")
                __result = __instance.m_MaskValue.TryGetValue("levelDesigner", out var d)
                    ? d.ToString()
                    : __instance.levelDesigner;
    ...
    internal class PnlRankPatch
    {
        ...
        {
            var repairingComp = __instance.repairing.GetComponent<Text>();
            if (FirstRun)
            {
    ...
    [HarmonyPatch(typeof(DataHelper), nameof(DataHelper.CheckMusicUnlockMaster), typeof(MusicInfo), typeof(bool))]
    internal class CheckUnlockMasterPatch
    {
        private static bool Prefix(MusicInfo musicInfo, ref bool __result)
        {
            ...
            if (GlobalDataBase.dbUi.ability == 0 && ability != 0)
            {
                ...
                var vanillaParse = musicInfo.difficulty3.TryParseAsInt(out var difficulty);
                var cond = !vanillaParse || ability >= difficulty;
                __result = cond || DataHelper.unlockMasters.Contains(uid) ||
                           SaveData.UnlockedMasters.Contains(AlbumManager.GetAlbumNameFromUid(uid)) ||
                           (!AlbumManager.GetByUid(musicInfo.uid)?.IsPackaged ?? false);
                return false;
            }
            ...
            var successParse = musicInfo.difficulty3.TryParseAsInt(out var diffNum);
            var abilityConditionOrGimmick = !successParse || ability >= diffNum;

            __result = abilityConditionOrGimmick ||
                       SaveData.UnlockedMasters.Contains(AlbumManager.GetAlbumNameFromUid(uid)) ||
                       !AlbumManager.GetByUid(musicInfo.uid).IsPackaged;
            return false;
        }
    }
    ...
    internal class UploadScorePatch
    {
        private static void Postfix(SceneUploadResultData resData)
        {
            ...
            SaveScore(resData.musicUid, resData.musicDifficulty, resData.score, resData.accuracy, resData.combo,
                resData.evaluate, resData.miss);
        }
    }
    ...
    internal class SetScorePatch
    {
        private static void Postfix(PnlVictory __instance)
        {
            ...
            if (!ModSettings.SavingEnabled ||
                !GlobalDataBase.dbBattleStage.musicUid.StartsWith($"{AlbumManager.Uid}-")) return;
    ...
    private delegate IntPtr RecordBattleArgsDelegate(IntPtr instance, IntPtr args, IntPtr isSuccess,
